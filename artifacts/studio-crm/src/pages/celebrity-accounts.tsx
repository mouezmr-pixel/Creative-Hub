...
import { UserCog, Eye, EyeOff, Pencil, Search, ShieldCheck } from "lucide-react";
...
async function fetchCelebrityAccounts(): Promise<CelebrityAccount[]> {
  const res = await fetch(`${BASE}/api/celebrity-accounts`, { credentials: "include" });
  if (!res.ok) throw new Error("Failed to fetch celebrity accounts");
  return res.json();
}

async function updateCelebrityAccount(id: number, data: Partial<{
  username: string;
  password: string;
}>): Promise<CelebrityAccount> {
  const res = await fetch(`${BASE}/api/celebrity-accounts/${id}`, {
    method: "PATCH",
...
function EditAccountDialog({ account, onClose }: { account: CelebrityAccount; onClose: () => void }) {
  const { t } = useLanguage();
  ...
  const mutation = useMutation({
    mutationFn: (data: Parameters<typeof updateCelebrityAccount>[1]) =>
      updateCelebrityAccount(account.id, data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["celebrity-accounts"] });
  ...
  const handleSave = () => {
    const data: Parameters<typeof updateCelebrityAccount>[1] = {};
...
export default function CelebrityAccounts() {
  const { t } = useLanguage();
  const { user } = useAuth();
  const [editingId, setEditingId] = useState<number | null>(null);
  const [search, setSearch] = useState("");

  const { data: accounts = [], isLoading } = useQuery<CelebrityAccount[]>({
    queryKey: ["celebrity-accounts"],
    queryFn: fetchCelebrityAccounts,
    ...
    <div className="space-y-6">
      <div>
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100 tracking-tight">{t("celebrityAccounts")}</h1>
        </div>
        <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm">
          {t("celebrityAccountsDesc")}
        </p>
      </div>
        ...
        <>
          <div className="relative">
            ...
            <Input
              ...
              placeholder={t("searchCelebrityAccounts")}
                    ...
                    <DialogContent className="sm:max-w-md rounded-2xl">
                      <DialogHeader>
                        <DialogTitle className="text-base font-bold">
                          {t("editCelebrityAccount")} — {account.name}
                        </DialogTitle>
                      </DialogHeader>
                      <EditAccountDialog
                        account={account}
